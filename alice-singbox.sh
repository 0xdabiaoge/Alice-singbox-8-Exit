...
GITHUB_PROXIES=(
    "https://gh-proxy.com/"
    "https://ghfast.top/"
    "https://hubproxy.cfapps.ap21.hana.ondemand.com/"
...
install_singbox_online() {
    ...
    echo ""
    read -rp "选择 [1-${#GITHUB_PROXIES[@]}] (默认 1): " proxy_choice
    proxy_choice=${proxy_choice:-1}
    
    if [ "$proxy_choice" -ge 1 ] && [ "$proxy_choice" -le "${#GITHUB_PROXIES[@]}" ]; then
    ...
    # 输入版本号
    echo ""
    read -rp "输入版本号 (默认 $DEFAULT_SINGBOX_VERSION): " version
    ...
    if ! curl -L --progress-bar -o "${tmp_dir}/${filename}" "$proxied_url"; then
        print_error "下载失败"
        return 1
    fi
    ...
    if [ ! -f "$bin_path" ]; then
        print_error "解压失败，找不到 sing-box 二进制文件"
        print_info "尝试查找..."
        find "$tmp_dir" -name "sing-box" -type f
        return 1
    fi
